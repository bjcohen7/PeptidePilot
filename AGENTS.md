...
- **`/start` is the ad landing page — treat it as a sealed room:** everything on it must be drug-class-free, INCLUDING surfaces exempted sitewide — its `<title>`, any JSON-LD, and its chrome. Keep it a bare route (no global nav/footer, whose anchor text carries drug terms); logo + flow + minimal legal/disclosure only.
...
- **Meta pixel/CAPI payloads:** event names and param VALUES are part of the compliance surface too. The quiz/results flow currently sends drug terms in pixel/CAPI params (`content_category: "GLP-1"`, `ViewContent.content_ids: [peptideId]`, CAPI `custom_data.peptideName`) — left intentionally pending a tracking-vs-compliance decision; do not add NEW drug-termed pixel params, and sanitize existing ones only with an explicit call (it affects conversion optimization).
