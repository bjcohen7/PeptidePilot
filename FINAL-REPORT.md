...
## Experiment annotation — email CTA restructure: direct provider deep links + hop page (2026-07-23)

**Change date: 2026-07-23.** Four emails now deep-link their primary CTA straight to the matched provider instead of the results page: `email_2_cost` ("See {Provider}'s pricing →", position `email_cost`), `email_4_process` ("Start my 10-minute intake →", `email_process`), `email_6_closer` ("Claim my match at {Provider} →", `email_closer`), and `nudge_still_deciding` ("Pick up where I left off →", `email_nudge`). Primary URL: `/go/{providerSlug}/{publicId}?position=email_{tag}&surface=email`; each carries a quiet secondary text link ("or review your full breakdown") to the exact results URL. `email_0/1/3/5`, all backfills, and `post_conversion` are unchanged (email_1 stays results-primary — its "nobody pays us for placement" copy forbids a direct affiliate exit).

**Guard:** the deep link is built in the worker's personalization ONLY when the lead's `topMatch.slug` resolves to an ACTIVE provider row; otherwise the primary button falls back to the results URL with the pre-restructure copy. The `topMatch.slug || topPeptideMatch` fallback (which can yield a drug token like "semaglutide" in a URL) can never produce a deep link.

**Hop page (scanner-prefetch mitigation + email pixel coverage):** `/go/...&surface=email` no longer 302s on first request. It serves a minimal hop page that redirects ONLY via JS (or a visible "Continue →" human tap for no-JS; explicitly NO meta-refresh — scanners follow it) to the same URL + `&confirm=1`, which runs the classic log+302. Consequences for metric readers:

- **Email `clicked_at` (Resend) now includes direct provider exits**, not just results-page visits — a click on a deep-linked email CTA is an intent-to-provider signal, stronger than the old results-page click.
- **`provider_click_logs` rows with `source_surface='email'` are real post-JS/tap confirms**, not scanner prefetches (a scanner GET of the raw link logs nothing).
...
## Experiment annotation — BMI calculator + evidence tie-break (2026-07-23, commit 35ed7ee)

**q5 SEMANTIC BREAK.** The self-report BMI tier question is replaced by a height(ft/in)/weight(lbs) **calculator** (one screen, numeric keyboards, 22-step count unchanged; /start bridge untouched). Computed BMI maps onto the EXISTING q5 answer indices — under-27 → idx 1, 27–30 → idx 2, 30+ → idx 3 — so scoring and downstream logic are unchanged; the calculator is a more accurate feeder into the same gate. **idx 0 is re-semanticized**: pre-2026-07-23 it meant "Under 25 (normal weight)"; from this date it means **"Prefer not to say"** (skip link) and carries a neutral prior (`bmi_qualifies: 2, fatloss: 1` — round 1 rejected this prior only because idx 0 then meant known-normal-weight). Raw height/weight are stored on the lead (`height_in`/`weight_lbs`, nullable; skip = NULL) and never displayed back as a medical judgment. **q5 answer-mix will shift from this date** (self-report guessing vs computed), so q5 distributions are not comparable across it.

**Evidence tie-break (published rule):** "when scores are within 10% for weight loss, clinical evidence ranks first." Implemented as an explicit, commented post-scoring step (`evidenceTieBreak` in `shared/scoring.ts`), NOT hidden weight tweaks: for weight-loss-intent leads (q0=1) whose non-GLP top scores within 10% of the GLP profile, the GLP peptide takes #1 and the displaced peptide drops to #2 with its real score. Displayed matchPercent is always the REAL computed percent — never inflated. **HARD BOUNDARY: never applies at q5 idx 1 (computed under-27)** — the clinical eligibility gate holds (live-proven with a tied-score vector: sema at 100% of top still not promoted at idx 1).

**Simulation (504 real 30-day leads):** WL GLP-top share 58.1% → 60.4% (tie-break only) → 63.1% (full change); 14 flips, all q5 idx 0 or idx 2, **zero at idx 1** and zero in non-WL cohorts. Historical idx-0 flips overstate the forward effect (old idx 0 = real under-25 users; new idx 0 = explicit skips only). Match distributions are not comparable across this date.

## Gala prefill — investigated 2026-07-27 and REJECTED (do not re-propose as-is)

Recon confirmed Gala's funnel honors `&email=`/`&phone=`/`&first_name=` query params (parsed into their `funnelData` localStorage state on load). **REJECTED because their page broadcasts the full URL — PII included — on page load to ~11 third parties** (Google Analytics ×4, Google Ads/DoubleClick, Stripe iframe, RevOffers/Katalys, and **Customer.io, which extracts email/phone/first_name as structured fields pre-signup**). Prefilling would hand our lead list to Gala's ESP on click, before any signup, and land PII permanently in ad-network logs. **Do not re-propose without solving the plaintext broadcast** (hashed or server-side handoff via their rep — parked for the AM conversation agenda). No code was changed; no prefill params exist anywhere in our codebase.

## LCP round 3 — hydration no longer wipes the prerendered DOM (2026-07-27, commit 64ebd1c)

The parked repaint fix, shipped behind a measured gate. Root cause: React's boot mount wiped the prerendered `/` and `/start` content into the router's Suspense fallback while the lazy route chunk resolved, so mobile-4G LCP was gated on the post-hydration repaint. Fix (no hydrateRoot — App/AppPrerender trees differ): shared preloadable route components (`client/src/lib/lazyRoutes.tsx`) for `/`, `/start`, `/quiz`, `/match`; `main.tsx` awaits the active route's chunk (3s cap) BEFORE mounting, so the first render never suspends and the prerendered hero is replaced by identical content in one commit. Client-side navigations keep normal lazy + RouteFallback.

**Gate numbers (5-run medians, identical local harness, main vs branch):** `/start` LCP 6.7s → 2.9s (−3.8s; bar was ≥1.5s), score 74→91; `/` LCP 7.9s → 3.3s (−4.6s), score 51→87, **CLS 0.514 → 0.014** (the fallback wipe was also a large layout shift). **Live prod after deploy (devtools throttling = applied, not modeled): `/start` LCP 2.4–2.5s == FCP, scores 92–94** — the prerendered hero now IS the LCP; no repaint penalty. Note: Lighthouse *simulate* mode still reports ~9s on prod — a Lantern modeling artifact that ties LCP to the JS dependency chain; devtools mode and the paint timeline are the ground truth here. Functional gate passed: live bridge walk console-clean, Q1–Q3 + handoff + ProviderHandoff pixel (1053…) all intact.

**Business metric to watch (next few days): the /start Q1-start rate — expect it to rise from ~64% if load time was the true cause of the ~36% never-tap cohort. If it doesn't move, the leak is something else and we investigate fresh.**

## LCP round 4 — LogRocket retired, pixel deferred, taps honest (2026-07-27, commit d0ffc28)

Manus's third-party audit confirmed round 3 (no blank flash, /start CLS 0.00, paint ~3.6s on real 4G+4×CPU) but found the successor problem: buttons visible at ~3.6s were DEAD until ~13s (long tasks from fbevents, pixel config, LogRocket, gtag) — visible-but-frozen reads as broken and was the prime suspect for the never-tap cohort. Three changes:

1. **LogRocket RETIRED (deleted, not deferred).** index.html loader, `lib/logrocket.ts`, both identify call sites — zero `logrocket` hits in the built bundle. No Railway env vars existed for it (app id was hardcoded); nothing for Ben to remove. Debugging need is covered by bridge_q logging, click logs, pixel events, and the digest. Re-add deliberately if session replay is ever wanted again.
2. **Meta pixel loads politely.** The fbq queue stub installs at boot (every event fired pre-load is queued); `fbevents.js` injects after hydration + requestIdleCallback (4s cap). Proven in a prod-mode build AND live: BridgeQ1 fired before the script loaded transmitted with params intact once flushed; live walk shows BridgeQ1/Q2/Q3 + ProviderHandoff all reaching pixel 1053…. CAPI untouched.
3. **Pre-hydration tap buffer on /start Q1.** 409-byte inline script in the prerendered HTML: early taps get the pressed style + the answer stashed; Start.tsx consumes the stash on mount (records, fires BridgeQ1, advances to Q2). The invariant holds: buttons that look alive respond — tap-responsive from first paint (~2.3s), regardless of hydration timing.

**Prod before → after (devtools throttling):** /start TTI 6.7s → 5.9s, TBT 203 → ~150ms, LCP held 2.3s, CLS 0.000; LogRocket gone from the long-task list (was up to ~144ms exec). The 509ms-class task from Manus's profile attributed to the fbevents/pixel/LogRocket cluster — LogRocket deleted, pixel now post-idle. **Homepage CLS under heavy throttle (Manus 0.09): the hero subhead `<p>` reflows when Inter swaps in late — not trivially fixable without re-adding critical-path font bytes; documented, not fixed.**

**The nightly `/start` digest line now tests paint AND responsiveness together** — Q1-start rate movement from here reflects both fixes (round 3 paint + round 4 responsiveness). Local-build gotcha recorded: `node scripts/build.mjs` without `NODE_ENV=production` produces a DEV bundle with the pixel gated off; Railway builds are production and unaffected.

---

## 2026-08-04 — Protocol-briefing results page (partner mock adopted, honest-machine enforced) + lead first name

**Shipped (deployed, live by default):** `/results/:publicId` now renders a "Protocol Briefing" page structured on the partner mock — hero ratio-problem synthesis → real-answer stat strip → two-paths comparison → three-phase 12-month plan → component explainers → four failure modes → five tracked metrics → provider close with CTA `/go/{slug}/{publicId}?position=results_sp` (fires ProviderHandoff on pointerdown; `results_sp` = 10 chars, fits `position` varchar(16)). Flag: `RESULTS_PAGE_MODE` env (default `briefing`, `classic` restores prior pages), plus `?rmode=` per-request override for testing.

**Honest-machine resolutions vs the mock:** (a) promo section config-gated on `providers.promo_code` + `GALA_PROMO_TERMS` — unset = fully hidden; PILOT30/"$99×2mo" never shipped. (b) Intake-code theater replaced with the lead's real publicId short-ref + "results stay live at {link}". (c) Tesamorelin removed; combination-therapy interest moved to consult questions. (d) Phase expectation numbers recomputed from the lead's actual weight against STEP/SURMOUNT trajectories (4–6% wk 8, 10–14% wk 28, 15–20% at 12 mo — the mock's year-end range overshot semaglutide literature), labeled "Typical range (clinical trials):" with a section-end disclaimer; the 25–40% lean-mass stat carries its citations (Wilding 2021 STEP 1; Jastreboff 2022 SURMOUNT-1).

**Two judgment calls beyond the spec (flagging for Ben):** (1) **WL-relevance gate** — the briefing is weight-loss framed end to end, but every lead gets provider matches; rendering it for a Sermorelin/anti-aging lead would misstate their answers. Briefing renders only when topPeptideMatch is semaglutide/tirzepatide OR primary goal is weight loss; others keep classic/verdict (verified live: non-WL lead falls through). (2) **Stale-price whyMatch filter** — persisted whyMatch rows from the $179 era contradicted the live $129 pricing line on one page; rows whose $ figures don't match live price are dropped.
